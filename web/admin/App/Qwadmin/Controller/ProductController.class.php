...
class ProductController extends ComController
{
    ...
    public function update()
    {
        ...
        $data = M('whh_product',null)->field('prod_id')->where("prod_phone = '%s' and prod_name = '%s'",array($list['prod_phone'],$list['prod_name']))->find();
        if (!$prod_id) {
            if ($list['prod_phone']) {
                if ($data) {
                    $this->error('产品库已有该产品，请勿重复添加！');
                }
                $uid = M('whh_user',null)->field('uid,user_type')->where('mobile = %s',array($list['prod_phone']))->find();
                $list['prod_uid'] = $uid['uid'];
                if (!$list['prod_uid']) {
