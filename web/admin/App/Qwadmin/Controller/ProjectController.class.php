...
class ProjectController extends ComController
{
    ...
    public function update()
    {
        ...
        $data = M('project','whh_')->field('proj_id')->where("proj_phone = '%s' and proj_name = '%s'",array($list['proj_phone'],$list['proj_name']))->find();
        if ($proj_id) {
            ...
            if ($proj_phone) {
                ...
                $uid = M('whh_user',null)->field('uid,user_type')->where('mobile = %s',array($proj_phone))->find();
                $list['proj_uid'] = $uid['uid'];
                if (!$list['proj_uid']) {
